...
def ensure_local_llm(data_dir: str) -> dict:
    ...
    Returns:
        dict: {"downloaded": bool, "model_path": str, "error": str}
    """
    ...
    result = {"downloaded": False, "model_path": "", "error": ""}
    ...
    if not gguf_files:
        ...
        logger.info("正在下载本地模型 (~760MB)...")
        try:
            sys.stderr.write("[OAA] 下载本地模型 BitCPM4-1B-q4_0 (~760MB)...\n")
    ...
    if not server_exe.exists():
        cpu_url = (
            "https://github.com/ggml-org/llama.cpp/releases/download/b4319"
            "/llama-b4319-bin-win-msvc-x64.zip"
        )
        sys.stderr.write("[OAA] 下载 llama-server (CPU, ~15MB)...\n")
        try:
                ...
                with zipfile.ZipFile(tmp.name) as zf:
                    for member in zf.namelist():
                        if member.endswith("llama-server.exe"):
                            ...
                            extracted = llama_dir / member.replace("\\", "/").split("/")[-1]
                            # Move to expected path
                            extracted_path = llama_dir / member
                            if extracted_path.exists() and extracted_path != server_exe:
                                shutil.move(str(extracted_path), str(server_exe))
                            elif not server_exe.exists():
                                # Try finding in subdir
                                for p in llama_dir.rglob("llama-server.exe"):
                                    if p != server_exe:
                                        shutil.move(str(p), str(server_exe))
                                        break
                os.unlink(tmp.name)
        except Exception as e:
