...
class RepairLoop:
    ...
    def _build_feed_prompt(self, plan: RepairPlan) -> str:
        ...
        parts.extend([
            ...
            "5. **代码修复优先用 apply_patch** — 当需要修改 Python 代码修复 bug 时：",
            "   a. `read_own_source` 读目标源码，定位问题函数/方法",
            "   b. 用 `apply_patch` 而非 `self_improve`：apply_patch 不修改源码文件，",
            "      在 .exe 环境也能工作，重启后自动重新生效",
            "   c. apply_patch 参数：target_module 用模块路径，target_attr 用 Class.method，",
            "      new_code 写完整函数定义（含 def 行和签名）",
            "   d. `reload_module` 热重载使补丁立即生效",
            "   e. 只有需要永久修改源码时才用 `self_improve`",
            "6. 验证修复 — 重新检查原始问题是否解决",
            "7. 输出结果 — 总结：搜索了什么、为什么选这个、试运行结果、问题是否解决",
            "",
            "约束：",
            ...
            "- 每次修改会自动记录备份，无需担心改坏（apply_patch 会自动保存原始代码用于回滚）",
        ])
        return "\n".join(parts)
